...
export default async function KnjigaUtisakaPage({
  params,
}: {
    ...
    <main className="min-h-screen bg-[#f4efe5] px-6 py-10 text-[#15130f]">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="py-10">
          <p className="text-sm uppercase tracking-[0.22em] text-[#b8944d]">Trust signal</p>
          <h1 className="mt-5 max-w-4xl text-5xl font-semibold leading-tight">
            {lang === "de" ? "Gästebuch" : lang === "en" ? "Guestbook" : "Knjiga utisaka"}
          </h1>
          <p className="mt-8 max-w-3xl text-lg leading-8 text-[#706a5d]">
            Moderisan prostor za pitanja, utiske, iskustva i predloge. AI može
            pomoći u moderaciji, ali čovek donosi konačnu odluku.
          </p>
          ...
          <form className="mt-10 grid max-w-2xl gap-4 rounded-3xl border border-black/10 bg-[#fffaf1] p-8">
            ...
            <input className="rounded-xl border border-black/10 bg-[#fbf7ef] p-4" placeholder="Tema / Topic" />
            <textarea className="min-h-40 rounded-xl border border-black/10 bg-[#fbf7ef] p-4" placeholder="Poruka / Message" />
            <label className="flex gap-3 text-sm text-[#706a5d]">
              <input type="checkbox" />
              Saglasan/saglasna sam da se poruka moderira pre eventualne objave.
            </label>
