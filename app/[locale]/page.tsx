...
const copy: Record<Locale, Copy> = {
  sr: {
    ...
    hero: {
      ...
      subtitle: "DaniniNet povezuje digitalni marketing, affiliate tokove, SEO sadržaj, DaniniHub AI dijalog i digitalne proizvode u sistem koji ima dokaz, granice i sledeći korak.",
      ...
      panelText: "DaniniHub metoda se koristi da ideja ne ode direktno u prodaju bez pitanja, dokaza, granica i jasnog artifacta.",
  ...
  de: {
    ...
    hero: { eyebrow: "DaniniNet · Income · Intelligence · Health", title: "Digitale Systeme rund um Ideen mit Marktpotenzial.", subtitle: "DaniniNet verbindet digitales Marketing, Affiliate-Flows, SEO-Inhalte, DaniniHub KI-Dialog und digitale Produkte zu einem System mit Belegen, Grenzen und nächstem Schritt.", primary: "Produkte ansehen", secondary: "DaniniHub Methode ansehen", proof: "Digitaler Verkauf von Standorten · echtes DaniniNet Produkt", signals: ["Digitales Produkt", "Gateway-Logik", "SEO + Trust", "Gumroad Flow"], panelTitle: "KI-unterstütztes System, kein generischer Text.", panelText: "Die DaniniHub Methode verhindert, dass eine Idee ohne Fragen, Belege, Grenzen und klares Artefakt direkt in den Verkauf geht.", panelSteps: [["01", "Frage vor Inhalt"], ["02", "Beleg vor Aussage"], ["03", "Grenze vor CTA"], ["04", "Artefakt vor Kampagne"]] },
  ...
  en: {
    ...
    hero: { eyebrow: "DaniniNet · Income · Intelligence · Health", title: "Digital systems around ideas with market potential.", subtitle: "DaniniNet connects digital marketing, affiliate flows, SEO content, DaniniHub AI dialogue and digital products into a system with proof, boundaries and a next step.", primary: "View products", secondary: "See the DaniniHub method", proof: "Digital Location Sales · real DaniniNet product", signals: ["Digital product", "Gateway logic", "SEO + trust", "Gumroad flow"], panelTitle: "AI-assisted system, not generic copy.", panelText: "The DaniniHub method prevents an idea from going straight into sales without questions, proof, boundaries and a clear artifact.", panelSteps: [["01", "Question before content"], ["02", "Proof before claim"], ["03", "Boundary before CTA"], ["04", "Artifact before campaign"]] },
    ...
    <main className="min-h-screen bg-[#08090B] text-[#F8F3E8]">
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_18%_10%,rgba(197,163,90,0.18),transparent_32%),radial-gradient(circle_at_82%_18%,rgba(45,91,141,0.20),transparent_34%),linear-gradient(180deg,#05070A_0%,#071326_62%,#EDE1CE_62%,#EFE6D6_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-7 md:px-8">
          ...
          <div className="grid gap-12 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:py-24">
            <div><p className="mb-6 inline-flex rounded-full border border-[#C5A35A]/35 bg-white/[0.05] px-4 py-2 text-sm text-[#E9D8A8]">{t.hero.eyebrow}</p><h1 className="max-w-4xl text-5xl font-semibold leading-[1.02] tracking-tight md:text-7xl">{t.hero.title}</h1><p className="mt-8 max-w-3xl text-lg leading-8 text-[#D9E2EA]">{t.hero.subtitle}</p><div className="mt-10 flex flex-wrap gap-4"><Link href={path(lang, t.routes.products)} className="rounded-full bg-[#F8F3E8] px-6 py-3 text-sm font-semibold text-[#071326]">{t.hero.primary}</Link><Link href={path(lang, t.routes.method)} className="rounded-full border border-white/18 px-6 py-3 text-sm font-medium text-white">{t.hero.secondary}</Link></div><div className="mt-10 grid max-w-3xl gap-3 sm:grid-cols-2 lg:grid-cols-4">{t.hero.signals.map((signal) => <span key={signal} className="rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-3 text-sm text-[#D9E2EA]">{signal}</span>)}</div></div>
            <Card className="relative overflow-hidden p-5"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-[#C5A35A]/20 blur-3xl" /><div className="relative grid gap-5 lg:grid-cols-1"><div className="mx-auto w-full max-w-[360px] overflow-hidden rounded-[1.6rem] border border-[#C5A35A]/25 bg-[#05070A] p-3"><Image src="/images/products/digitalna-prodaja-lokacije-cover.svg" alt="Digitalna prodaja lokacije — DaniniNet PDF vodič" width={900} height={1272} priority className="h-auto w-full rounded-[1.2rem]" /></div><div className="rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-5"><p className="text-xs uppercase tracking-[0.22em] text-[#C5A35A]">{t.hero.proof}</p><h2 className="mt-4 text-2xl font-semibold leading-tight text-white">{t.hero.panelTitle}</h2><p className="mt-3 text-sm leading-7 text-[#D9E2EA]">{t.hero.panelText}</p><div className="mt-5 grid gap-3 sm:grid-cols-2">{t.hero.panelSteps.map(([num, label]) => <div key={num} className="rounded-2xl border border-white/10 bg-[#071326] p-4"><span className="text-xs text-[#C5A35A]">{num}</span><p className="mt-1 text-sm font-semibold text-white">{label}</p></div>)}</div></div></div></Card>
          </div>
        </div>
      </section>
