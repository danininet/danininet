      ...
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_18%_10%,rgba(197,163,90,0.18),transparent_32%),radial-gradient(circle_at_82%_18%,rgba(45,91,141,0.20),transparent_34%),linear-gradient(180deg,#05070A_0%,#071326_72%,#EDE1CE_72%,#EFE6D6_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-7 md:px-8">
          ...
          <div className="grid gap-10 py-12 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start lg:py-16">
            <div className="min-w-0"><p className="mb-6 inline-flex rounded-full border border-[#C5A35A]/35 bg-white/[0.05] px-4 py-2 text-sm text-[#E9D8A8]">{t.hero.eyebrow}</p><h1 className="max-w-4xl text-4xl font-semibold leading-[1.04] tracking-tight md:text-6xl lg:text-[4.8rem]">{t.hero.title}</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-[#D9E2EA]">{t.hero.subtitle}</p><div className="mt-8 flex flex-wrap gap-4"><Link href={path(lang, t.routes.products)} className="rounded-full bg-[#F8F3E8] px-6 py-3 text-sm font-semibold text-[#071326]">{t.hero.primary}</Link><Link href={path(lang, t.routes.method)} className="rounded-full border border-white/18 px-6 py-3 text-sm font-medium text-white">{t.hero.secondary}</Link></div><div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-2 lg:grid-cols-4">{t.hero.signals.map((signal) => <span key={signal} className="rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-3 text-sm text-[#D9E2EA]">{signal}</span>)}</div></div>
