...
const copy: Record<Locale, PageCopy> = {
  sr: {
    ...
    bonusItems: ["Pre-launch checklist", "Investor Brief Outline", "Homepage Structure Template", "Lead Calculator spec", "Email Templates", "Visual Asset Checklist", "Public / Private Information Map", "Metrics Tracker", "Disclaimer Library", "DaniniHub paket pitanja i dijalog metod"],
  ...
  de: {
    ...
    bonusItems: ["Pre-launch checklist", "Investor Brief Outline", "Homepage Structure Template", "Lead Calculator spec", "Email Templates", "Visual Asset Checklist", "Public / Private Information Map", "Metrics Tracker", "Disclaimer Library", "DaniniHub Fragen- und Dialogmethodik"],
  ...
  en: {
    ...
    bonusItems: ["Pre-launch checklist", "Investor Brief Outline", "Homepage Structure Template", "Lead Calculator spec", "Email Templates", "Visual Asset Checklist", "Public / Private Information Map", "Metrics Tracker", "Disclaimer Library", "DaniniHub questions and dialogue method"],
...
export default async function ProductPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <SiteShell locale={lang} currentPath={localizedPath(lang, "dpl")}>
      ...
      <section className="relative overflow-hidden bg-[#07142b] text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_8%,rgba(215,180,106,0.18),transparent_32%),radial-gradient(circle_at_84%_14%,rgba(68,130,190,0.20),transparent_34%)]" /><div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.02fr_0.98fr] lg:items-center"><div><p className="text-sm uppercase tracking-[0.24em] text-[#d7b46a]">{t.eyebrow}</p><h1 className="mt-5 max-w-4xl text-5xl font-semibold leading-tight tracking-tight md:text-6xl">{t.title}</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-slate-300">{t.subtitle}</p><div className="mt-9 flex flex-wrap gap-4"><a href={checkoutUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-[#f7fbff] px-7 py-4 text-sm font-semibold text-[#07142b]">{t.buy}</a><a href="#inside" className="rounded-full border border-white/15 px-7 py-4 text-sm font-medium text-white">{t.inside}</a></div><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{t.heroBadges.map((badge) => <span key={badge} className="rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm text-slate-300">{badge}</span>)}</div><HeroFlow items={t.visualFlow} /><p className="mt-5 text-sm text-slate-400">{t.micro}</p></div><aside className="rounded-[2.4rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl"><div className="grid gap-5 md:grid-cols-[0.62fr_0.38fr] md:items-center lg:grid-cols-1 xl:grid-cols-[0.62fr_0.38fr]"><Image src="/images/products/digitalna-prodaja-lokacije-cover.svg" alt="Digitalna prodaja lokacije cover" width={900} height={900} className="rounded-[2rem] bg-[#0a1834]" priority /><div className="rounded-[1.8rem] bg-[#fffaf1] p-6 text-[#15130f]"><p className="text-sm uppercase tracking-[0.2em] text-[#9a7a38]">{t.price}</p><p className="mt-2 text-sm text-[#706a5d]">{t.regular}</p><ul className="mt-6 space-y-3">{t.heroValueStack.map(([k, v]) => <li key={k} className="border-t border-black/10 pt-3"><strong>{k}</strong><span className="mt-1 block text-sm text-[#706a5d]">{v}</span></li>)}</ul></div></div></aside></div></section>
      <Section tone="cream"><div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="text-sm uppercase tracking-[0.22em] text-[#9a7a38]">Artifact</p><h2 className="mt-3 text-4xl font-semibold">{t.artifactTitle}</h2><p className="mt-5 text-lg leading-8 text-[#706a5d]">{t.artifactText}</p></div><div className="grid gap-4 md:grid-cols-2">{t.artifactSteps.map(([n, txt]) => <article key={n} className="rounded-[2rem] bg-white p-6 shadow-sm"><span className="text-sm font-semibold text-[#9a7a38]">{n}</span><p className="mt-3 text-[#706a5d]">{txt}</p></article>)}</div></div></Section>
      <Section><div className="grid gap-8 md:grid-cols-2"><article className="rounded-[2rem] border border-black/10 bg-white p-8"><h2 className="text-3xl font-semibold">{t.problemTitle}</h2><p className="mt-5 text-lg leading-8 text-[#706a5d]">{t.problemText}</p></article><article className="rounded-[2rem] border border-black/10 bg-[#07142b] p-8 text-white"><h2 className="text-3xl font-semibold">{t.productTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-300">{t.productText}</p></article></div></Section>
      <Section tone="cream"><div className="grid gap-8 md:grid-cols-2"><ChecklistColumn title={t.audienceTitle} items={t.audience} mark="✓" /><ChecklistColumn title={t.notForTitle} items={t.notFor} mark="—" /></div></Section>
      <Section><div id="inside"><p className="text-sm uppercase tracking-[0.22em] text-[#9a7a38]">Package</p><h2 className="mt-3 text-4xl font-semibold">{t.includesTitle}</h2><p className="mt-4 max-w-3xl text-lg leading-8 text-[#706a5d]">{t.includesIntro}</p><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{t.includes.map((item) => <article key={item.title} className="rounded-[2rem] border border-black/10 bg-white p-7"><LineIcon type={item.key} /><h3 className="mt-5 text-2xl font-semibold">{item.title}</h3><p className="mt-3 leading-7 text-[#706a5d]">{item.text}</p></article>)}</div><div className="mt-10 rounded-[2rem] bg-[#07142b] p-8 text-white"><h3 className="text-3xl font-semibold">{t.bonusTitle}</h3><div className="mt-6 grid gap-3 md:grid-cols-2">{t.bonusItems.map((item) => <span key={item} className="rounded-2xl bg-white/[0.08] px-4 py-3 text-sm text-slate-200">{item}</span>)}</div></div></div></Section>
      <Section tone="dark"><div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="text-sm uppercase tracking-[0.22em] text-[#d7b46a]">Method</p><h2 className="mt-3 text-4xl font-semibold">{t.methodTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-300">{t.methodText}</p></div><div className="grid gap-4 md:grid-cols-3">{t.methodSteps.map((step, index) => <div key={step} className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6"><span className="text-[#d7b46a]">0{index + 1}</span><p className="mt-3 text-xl font-semibold">{step}</p></div>)}</div></div></Section>
      <Section><div className="rounded-[2.4rem] border border-black/10 bg-white p-8 md:p-10"><p className="text-sm uppercase tracking-[0.22em] text-[#9a7a38]">Proof</p><h2 className="mt-3 text-4xl font-semibold">{t.proofTitle}</h2><p className="mt-5 max-w-4xl text-lg leading-8 text-[#706a5d]">{t.proofText}</p><a href={calijePublicUrl} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex rounded-full bg-[#07142b] px-6 py-3 text-sm font-semibold text-white">{t.calijeLink}</a></div></Section>
      <Section tone="cream"><div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div><h2 className="text-4xl font-semibold">{t.purchaseTitle}</h2><ol className="mt-6 space-y-3 text-[#706a5d]">{t.purchaseSteps.map((step) => <li key={step} className="rounded-2xl bg-white p-4">{step}</li>)}</ol><a href={checkoutUrl} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex rounded-full bg-[#07142b] px-7 py-4 text-sm font-semibold text-white">{t.buy}</a></div><div><h2 className="text-4xl font-semibold">{t.faqTitle}</h2><div className="mt-6 space-y-4">{t.faq.map(([q, a]) => <details key={q} className="rounded-2xl bg-white p-5"><summary className="cursor-pointer font-semibold">{q}</summary><p className="mt-3 text-[#706a5d]">{a}</p></details>)}</div></div></div></Section>
      <Section tone="dark"><div className="grid gap-8 lg:grid-cols-[1fr_0.8fr]"><div><h2 className="text-4xl font-semibold">{t.finalTitle}</h2><p className="mt-5 text-slate-300">{t.disclaimer}</p></div><div className="flex flex-col justify-center gap-4"><a href={checkoutUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-[#f7fbff] px-7 py-4 text-center text-sm font-semibold text-[#07142b]">{t.buy}</a><Link href={path(lang, "support")} className="rounded-full border border-white/15 px-7 py-4 text-center text-sm font-medium text-white">Support</Link><Link href={path(lang, "blog")} className="text-center text-sm text-slate-300 underline">Blog / Kontext lesen</Link></div></div></Section>
    </SiteShell>
  );
}
