...
type Card = {
  ...
  status: string;
  cta: string;
  href: "dpl" | "miniAudit" | "newsletter" | "health";
  points: string[];
};
...
const miniAuditPath: Record<Locale, string> = {
...
const copy: Record<Locale, {
  ...
  sr: {
    ...
    text: "Na ovoj strani ne mešamo proizvode. Kupac DPL paketa dobija PDF vodič i Bonus Pack. DPL analiza ide preko posebnog upita. Affiliate i health/water pravci nisu deo DPL paketa.",
    ...
    offerText: "Zaključana ponuda ima dva nivoa: digitalni proizvod koji se kupuje odmah i DPL analiza koja se traži preko posebne stranice za upit.",
    cards: [
      {
      ...
      },
      {
        ...
        text: "Početni mini audit za vlasnike parcela, poslovnih prostora ili zakupnih površina: javni/privatni sloj, rizici, lead logika i sledeći koraci.",
        status: "Posebna stranica · nije Gumroad proizvod",
        cta: "Zatraži DPL analizu",
        href: "miniAudit",
        points: ["poseban upit", "kratka procena", "bez automatizovanih obećanja", "pisani okvir pre šireg rada"],
      },
    ],
    ...
    futureCards: [
      { label: "Health / Water · poseban projekat", title: "DaniniLans — voda i zdrav stil života", text: "Poseban health/water sloj sa jasnim granicama tvrdnji. Nije deo DPL paketa i ne sme se mešati sa prodajom lokacija.", status: "Poseban projekat · sledeća faza", cta: "Pogledaj pravac", href: "health", points: ["voda", "zdrav stil života", "health disclaimer", "DACH sadržaj kasnije"] },
      { label: "Affiliate · posebno označeno", title: "Hostinger, Amazon, WELLAN/UMH i drugi pravci", text: "Affiliate proizvodi idu samo kroz jasno označene preporuke, bez mešanja sa DPL isporukom i bez medicinskih ili finansijskih obećanja.", status: "Inventar · nije deo DPL kupovine", cta: "Prati razvoj", href: "newsletter", points: ["affiliate disclosure", "link audit pre objave", "bez hype-a", "posebne stranice"] },
    ],
  ...
  de: {
    ...
    text: "Der Käufer des DPL-Pakets erhält PDF-Leitfaden und Bonus Pack. Eine individuelle Standortanalyse wird separat angefragt.",
    ...
    offerText: "Das Angebot ist getrennt: digitales Produkt zum Sofortkauf und DPL Analyse über eine separate Anfrage-Seite.",
    cards: [
      ...
      { label: "Dienstleistung · Anfrage", title: "DPL Standortpotenzial-Analyse", text: "Erster Mini-Audit für konkrete Standorte: öffentliche/private Ebene, Risiken, Lead-Logik und nächste Schritte.", status: "Separate Seite · kein Gumroad Produkt", cta: "DPL Analyse anfragen", href: "miniAudit", points: ["separate Anfrage", "kurze Prüfung", "keine automatischen Versprechen", "schriftlicher Rahmen"] },
    ],
  ...
  en: {
    ...
    text: "The DPL buyer receives the PDF guide and Bonus Pack. Individual location analysis is requested separately.",
    ...
    offerText: "The offer is separated: a digital product for instant purchase and DPL analysis through a separate inquiry page.",
    cards: [
      ...
      { label: "Service · inquiry", title: "DPL location potential analysis", text: "Initial mini audit for concrete locations: public/private layer, risks, lead logic and next steps.", status: "Separate page · not a Gumroad product", cta: "Request DPL analysis", href: "miniAudit", points: ["separate inquiry", "short review", "no automated promises", "written frame"] },
    ],
...
function pathFor(lang: Locale, href: Card["href"]) {
  ...
  if (href === "miniAudit") return miniAuditPath[lang];
    ...
    <div className="mt-8 grid gap-6 md:grid-cols-2">
      {cards.map((card) => (
        <article key={card.title} className="rounded-[2rem] border border-black/10 bg-white p-7 shadow-sm">
          ...
          <ul className="mt-5 grid gap-2 text-sm text-[#706a5d]">{card.points.map((point) => <li key={point}>• {point}</li>)}</ul>
...
export default async function ProductsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <SiteShell locale={lang} currentPath={localizedPath(lang, "products")}>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10"><div className="rounded-[2rem] border border-[#226bbf]/20 bg-[#e8f4ff] p-6 text-[#07142b]"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#226bbf]">DaniniHub OS artifact layer</p><p className="mt-3 text-lg leading-8">{t.artifactNote}</p></div></section>
      <section className="mx-auto max-w-7xl px-6 pb-16"><article className="grid gap-8 rounded-[2.5rem] bg-[#07142b] p-8 text-white lg:grid-cols-[0.78fr_1.22fr] lg:items-center lg:p-10"><div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-4"><Image src="/images/products/digitalna-prodaja-lokacije-cover.svg" alt="Digitalna prodaja lokacije cover" width={900} height={1272} className="mx-auto h-auto w-full max-w-[330px] rounded-[1.4rem]" /></div><div><p className="text-sm uppercase tracking-[0.22em] text-[#d7b46a]">{t.featured}</p><h2 className="mt-4 text-4xl font-semibold leading-tight md:text-5xl">{t.dplTitle}</h2><p className="mt-5 max-w-3xl leading-8 text-slate-300">{t.dplText}</p><p className="mt-5 text-lg font-semibold text-[#d7b46a]">{t.price}</p><p className="mt-3 text-sm uppercase tracking-[0.2em] text-slate-400">{t.systemTitle}</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-7">{t.system.map((item, index) => <span key={item} className="rounded-2xl border border-white/10 bg-white/[0.05] px-3 py-3 text-center text-xs text-slate-300"><strong className="block text-[#d7b46a]">0{index + 1}</strong>{item}</span>)}</div><div className="mt-7 rounded-[1.5rem] border border-white/10 bg-white/[0.05] p-5"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#d7b46a]">{t.includeTitle}</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{dplIncludes.map((item) => <span key={item} className="text-sm leading-6 text-slate-300">• {item}</span>)}</div></div><Link href={localizedPath(lang, "dpl")} className="mt-8 inline-flex rounded-full bg-[#d9ecff] px-6 py-3 text-sm font-semibold text-[#07142b]">{t.dplCta}</Link></div></article></section>
      <section className="mx-auto max-w-7xl px-6 pb-16"><div className="grid gap-7 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="text-sm uppercase tracking-[0.22em] text-[#226bbf]">{t.offerTitle}</p><h2 className="mt-4 text-4xl font-semibold leading-tight">DPL paket + DPL analiza</h2></div><p className="text-lg leading-9 text-[#706a5d]">{t.offerText}</p></div><CardGrid lang={lang} cards={t.cards} /></section>
      <section className="mx-auto max-w-7xl px-6 pb-16"><div className="grid gap-7 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="text-sm uppercase tracking-[0.22em] text-[#226bbf]">{t.futureTitle}</p><h2 className="mt-4 text-4xl font-semibold leading-tight">DaniniLans · Affiliate · Health / Water</h2></div><p className="text-lg leading-9 text-[#706a5d]">{t.futureText}</p></div><CardGrid lang={lang} cards={t.futureCards} /></section>
      <section className="mx-auto max-w-7xl px-6 pb-16"><div className="grid gap-6 rounded-[2rem] bg-[#07142b] p-8 text-white lg:grid-cols-[0.72fr_1.28fr]"><div><p className="text-sm uppercase tracking-[0.22em] text-[#d7b46a]">DaniniHub OS</p><h2 className="mt-4 text-4xl font-semibold leading-tight">{t.methodTitle}</h2></div><div className="grid gap-4 md:grid-cols-2">{t.methodSteps.map(([num, text]) => <div key={num} className="rounded-2xl border border-white/10 bg-white/[0.05] p-5"><span className="text-sm font-semibold text-[#d7b46a]">{num}</span><p className="mt-2 leading-7 text-slate-300">{text}</p></div>)}</div></div></section>
      <section className="mx-auto max-w-7xl px-6 pb-16"><div className="rounded-[2rem] border border-black/10 bg-white p-8 shadow-sm"><h2 className="text-3xl font-semibold">{t.disclaimerTitle}</h2><p className="mt-4 leading-8 text-[#706a5d]">{t.disclaimerText}</p></div></section>
    </SiteShell>
  );
}
