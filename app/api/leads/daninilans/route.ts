...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.BREVO_API_KEY || !process.env.BREVO_DANINILANS_LIST_ID) {
      return NextResponse.json({ ok: false, error: "Lead service is not configured" }, { status: 500 });
    }

    const lead = validation.data;
    const listId = Number(process.env.BREVO_DANINILANS_LIST_ID);
    ...
    console.error("DaniniLans lead registration failed", error);
